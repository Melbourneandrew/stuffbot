...
const ITEM_PROMPT = "What type of item is this and what would be its current resale value? Make your best effort to identify the item even if the image quality isn't perfect. Provide a human-friendly, searchable name including the brand name if visible (e.g., 'Sony 55-inch TV', 'Nike running shoes', 'IKEA BILLY bookshelf'). Then estimate a conservative resale price in USD. Only classify as 'unknown' if it's completely impossible to make any reasonable guess about what the item is."
const ITEM_CLASS_DESCRIPTION = "A human-friendly, searchable name including the brand name if visible (e.g., 'Sony 55-inch TV', 'Nike running shoes', 'IKEA BILLY bookshelf'). Make your best effort to identify items even with imperfect image quality."
...
Deno.serve(async (req) => {
  try {
    ...
    const itemPromptWithClassifications = `${ITEM_PROMPT}`
