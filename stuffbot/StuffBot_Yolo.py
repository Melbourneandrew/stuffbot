...
class ObjectTracker:
    ...
    def start_continuous_detection(self):
        """Start continuous detection in a separate thread"""
        self.running = True
        self.detection_queue = Queue(maxsize=1)
        self.detection_thread = threading.Thread(target=self._continuous_detection_loop)
        self.detection_thread.daemon = True
        self.detection_thread.start()

    def stop_continuous_detection(self):
        """Stop the continuous detection thread"""
        self.running = False
        if hasattr(self, 'detection_thread'):
            self.detection_thread.join()

    def _continuous_detection_loop(self):
        """Internal method to run continuous detection"""
        while self.running:
            display_frame, detections = self.process_frame()
            ...
            if not self.detection_queue.empty():
                try:
                    ...
                    pass
            self.detection_queue.put((display_frame, detections))
            time.sleep(0.01)  # Small sleep to prevent thread from consuming too much CPU
...
def StuffBot_Detect(conf_threshold=0.5, save_images=True):
    ...
    tracker = ObjectTracker.get_instance(
        ...
        display_enabled=True  # Enable display
    )
    
    try:
        display_frame, detections = tracker.process_frame()
        
        # Display the frame with detections
        cv2.imshow('StuffBot Object Detection', display_frame)
        cv2.waitKey(10000)  # Show image for 2 seconds
...
def StuffBot_Continuous_Detect():
    """Function to continuously detect and display objects with tracking"""
    # Use singleton pattern to maintain one camera instance
    tracker = ObjectTracker.get_instance(
        display_enabled=True,  # Enable display for continuous detection
        save_images=False,     # Disable saving images for performance
        conf_threshold=0.5
    )
    
    try:
        tracker.start_continuous_detection()
        
        while True:
            if not tracker.detection_queue.empty():
                display_frame, detections = tracker.detection_queue.get()
        ...
        # Cleanup
        tracker.stop_continuous_detection()
        cv2.destroyAllWindows()
